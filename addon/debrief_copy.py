...
def cards_to_fix_caption(cards_to_fix: CardsToFix) -> str:
    if cards_to_fix.count == 0:
        return "No card repair stands out:\n- No repeated card-surface pattern stood out in this window."
    lines = [
...
def early_learning_caption(early_learning: EarlyLearning) -> str:
    ...
    lines = [
        ...
        "- Treat this as normal first-pass learning, not a card-edit signal; study extra only if these felt unfamiliar or clustered.",
    ]
    return "\n".join(lines)
